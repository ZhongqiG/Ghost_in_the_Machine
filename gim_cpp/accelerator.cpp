...
void accelerator(vector<fixed_16> w1, vector<fixed_16>  w2, vector<fixed_16>  bias_1, vector<fixed_16> bias_2, 
    ...
    vector<fixed_16> x1 = {0, 0, 1, 1};
    vector<fixed_16> x2 = {0, 1, 0, 1};
    vector<fixed_16> y = {0, 1, 1, 0};

    // setting up initial values for signals between layers
    vector<fixed_16> output_kmin1 = {0, 0};
    vector<fixed_16> delta_1 = {0, 0};
    vector<fixed_16> delta_2 = {0, 0};
    ...
    for (i = 0; i < NUM_ITERATIONS; i++) {
        ...
        for (j = 0; j < 4; j++) {
            ...
            vector<fixed_16> output_0 = {x1[j], x2[j]};
            ...
            vector<fixed_16> layer_1_forward = array(w1, bias_1, output_0, delta_1, lr, model, alpha, training);
            vector<fixed_16> output_1 = layer_1_forward[0];
            vector<fixed_16> layer_2_forward = array(w2, bias_2, output_1, delta_2, lr, model, alpha, training);
            vector<fixed_16> output_2 = layer_2_forward[0];
            ...
            inaccuracy = inaccuracy + abs(y[j] - output_2[0]);
            square_error = square_error + pow((y[j] - output_2[0]), 2);

            // run the backpropagation and update the array
            vector<fixed_16> layer_2_backward = array(w2, bias_2, output_1, delta_2, lr, model, alpha, training);
            vector<fixed_16> delta_1 = layer_2_backward[1];
            vector<fixed_16> weight_changes_2 = layer_2_backward[2];
            vector<fixed_16> bias_2_update = layer_2_backward[3];
            w2 = weight_changes_2;
            bias_2 = bias_2_update;
            vector<fixed_16> layer_1_backward = array(w1, bias_1, output_0, delta_1, lr, model, alpha, training);
            vector<fixed_16> delta_0 = layer_1_backward[1];
            vector<fixed_16> weight_changes_1 = layer_1_backward[2];
            vector<fixed_16> bias_1_update = layer_1_backward[3];
            w1 = weight_changes_1;
